...
object WorkerPoolActor:
  ...
  private final case class AggregatedBoids(updatedBoids: List[Boid]) extends WorkerPoolCommand

  def apply(boids: List[Boid], boidsModel: BoidsModel, gridActor: ActorRef[NeighboursRequest]): Behavior[WorkerPoolCommand] =
            ...
            context.spawnAnonymous(
              Aggregator[Reply, AggregatedBoids](
                ...
                aggregateReplies = replies => {
                  val lists = replies.collect { case BoidWorker.UpdatedBoids(bs) => bs }
                  AggregatedBoids(lists.flatten.toList)
                },
                timeout = 5.seconds
              )
            )
